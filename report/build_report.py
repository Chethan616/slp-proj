...
def build_markdown() -> str:
    ...
    a("")
    a("```")
    a("microphone --> recorded audio --> server")
    a("                                     |")
    a("             faster-whisper base.en (int8, CTranslate2)")
    a("                                     |  recognised text")
    a("             DistilBERT intent classifier, int8 ONNX (16-way softmax)")
    a("                                     |  intent + confidence")
    a("             recipe corpus lookup --> recognised speech + intent + reply shown")
    a("```")
    a("")
    ...
    a("### 2.2 Subset used")
    a("")
    a("This project uses one CLINC domain in full - kitchen and dining, 15 intents -")
    a("plus the out-of-scope class, giving **16 classes**. The selection is fixed in")
    a("code (`train/prepare_data.py`) and therefore reproducible.")
    a("")
    a("Taking a whole domain rather than a slice of several gives the assistant a")
    a("subject it can genuinely answer about, and makes the out-of-scope class more")
    a("important rather than less: a narrow assistant is asked things it cannot handle")
    a("far more often than a general one.")
    a("")
    a("| Area | Intents |")
    a("|---|---|")
    a("| Cooking | recipe, ingredients_list, ingredient_substitution, cook_time, meal_suggestion |")
    a("| Nutrition | calories, nutrition_info, food_last |")
    a("| Restaurants | restaurant_suggestion, restaurant_reviews, how_busy, restaurant_reservation, confirm_reservation, cancel_reservation, accept_reservations |")
    a("| - | oos (out of scope) |")
    a("")
    a("| Split | Utterances | Per in-scope intent | Out-of-scope |")
    a("|---|---:|---:|---:|")
    a(f"| Train | {counts['train']:,} | 100 | 250 |")
    a(f"| Validation | {counts['val']:,} | 20 | 75 |")
    a(f"| Test | {counts['test']:,} | 30 | 150 |")
    a("")
    a("The out-of-scope examples were subsampled from the 1,000 that CLINC provides.")
    a("Left at full size they would outnumber the 450 in-scope test utterances and")
    a("dominate every aggregate metric.")
    a("")
    a("### 2.3 Second dataset: the recipe corpus")
    a("")
    a("CLINC150 is a classification dataset and ships no replies, so a second corpus")
    a("supplies the answers. **recipes-with-nutrition** (datahiveai, Hugging Face) holds")
    a("**39,447 recipes** with per-recipe servings, energy, full nutrient breakdowns,")
    a("ingredient lists and dietary labels.")
    a("")
    a("It cannot train an intent classifier - it contains recipes, not user utterances -")
    a("but it serves as the knowledge base the food intents answer from. Asked how many")
    a("calories are in butter chicken, the assistant classifies the intent with")
    a("DistilBERT, matches the dish against 39,447 recipe names, and quotes the real")
    a("figure. Where no dish is named, or where the corpus holds nothing relevant, a")
    a("written template is used instead.")
    a("")
    a("The published file is 450 MB of nested JSON. `train/build_recipes.py` reduces it")
    a("to the fields the assistant quotes and writes 2.4 MB gzipped, small enough to")
    a("deploy alongside the models.")
    a("")
    ...
    if dep:
        ...
        a(f"   -> Linear(768, {len(labels)}) -> softmax over {len(labels)} classes")
        a("```")
        a("")
    ...
    if voice:
        ...
        a(f"{s['corpus_wer'] * 100:.0f} words in every 100 are recognised wrongly, and")
        a(f"{fmt_pct(s['perfect_transcriptions'])} of utterances come back with no errors")
        a("at all.")
        a("")
        if abs(s["accuracy_drop"]) < 1e-9:
            ...
            a(f"The intent classifier absorbs most of those errors: accuracy falls by")
            a(f"{s['accuracy_drop'] * 100:.1f} points when the input arrives as speech rather")
            a("than as text, because recognition errors tend to land on words that do not")
        ...
        a(f"roughly {1 / s['real_time_factor']:.1f} times faster than the audio was spoken.")
        a("")
    ...
    a("")
    a("It is deployed on **Streamlit Community Cloud**, which builds the app directly")
    a("from the GitHub repository. Both models run server-side, so the browser only")
    a("records audio and displays results.")
    a("")
    a("### 7.1 Removing PyTorch from the runtime")
    a("")
    onnx = metrics.get("distilbert_onnx_int8")
    a("Free hosting tiers have a fixed disk and memory budget, and the default Linux")
    a("PyTorch wheel pulls in roughly 2.5 GB of CUDA libraries that a CPU host will")
    a("never execute. Neither model actually needs PyTorch at inference time: Whisper")
    a("already runs on CTranslate2, and the fine-tuned classifier was exported to an")
    a("**int8-quantised ONNX graph** served by ONNX Runtime. PyTorch is therefore a")
    a("training-time dependency only.")
    a("")
    if onnx:
        a("| Property | PyTorch build | Deployed ONNX int8 build |")
        a("|---|---:|---:|")
        a(f"| Model size on disk | 268.0 MB | {onnx['size_mb']} MB |")
        a(f"| Test accuracy | {dep['test_accuracy']:.4f} | {onnx['test_accuracy']:.4f} |")
        a(f"| Macro F1 | {dep['test_macro_f1']:.4f} | {onnx['test_macro_f1']:.4f} |")
        a(f"| Relative inference speed | 1.0x | {onnx['speedup_vs_torch']}x |")
        a("")
        a("Quantisation is effectively lossless here: the int8 graph agrees with the")
        a(f"full-precision model on {fmt_pct(onnx['agreement_with_fp32'])} of test utterances,")
        a("and where they disagree the errors cancel out, leaving accuracy marginally")
        a(f"*higher* ({onnx['test_accuracy']:.4f} against {dep['test_accuracy']:.4f}) rather")
        a("than lower. The end-to-end voice figures in section 6.4 were measured on this")
        a("deployed build, not on the PyTorch one.")
        a("")
    a("The weights are pulled from a public model repository on the Hugging Face Hub at")
    a("startup, because Streamlit Community Cloud does not fetch Git LFS objects from")
    a("the source repository. Both models are loaded once and cached for the lifetime of")
    a("the container rather than per interaction, which would otherwise dominate the")
    a("latency budget.")
    a("")
    a("A `Dockerfile` is included so the same application can be reproduced or")
    a("self-hosted anywhere; it installs `libgomp1`, which CTranslate2 requires and the")
    a("slim Python base image omits.")
    a("")
    a("### 7.2 Interface")
    a("")
    a("The interface records from the microphone, then displays each turn as the")
    a("**recognised speech**, the **predicted intent** with its confidence and the two")
    a("runner-up intents, and the **generated reply**, along with the measured speech")
    a("recognition and inference latencies. A text tab is provided for machines without")
    a("a working microphone.")
    a("")
    a("A second front end is included in `app/main.py`: a FastAPI service exposing the")
    a("same pipeline programmatically, with a dependency-free HTML/JavaScript interface")
